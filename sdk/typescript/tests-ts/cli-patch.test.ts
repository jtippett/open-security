...
describe("scan and patch workflow", () => {
  ...
  test("passes the scan model, provider, and selected authentication to patching", async () => {
    ...
    // open-models fork: registry providers are emitted as one inline table.
    expect(
      invocation.find((argument) =>
        argument.startsWith("model_providers.fireworks="),
      ),
    ).toContain('env_key = "FIREWORKS_API_KEY"');
  });
