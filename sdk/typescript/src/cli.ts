...
export async function main(
  ...
  const cli = Cli.create("open-security", {
      ...
      async run({ options }) {
        try {
          exitCode = await runSkill(
            ...
            errorOutput,
            dependencies,
            // open-models fork: forward --provider
            { provider: options.provider },
          );
        } catch (error) {
          exitCode = 2;
    ...
    .command("patch", {
      ...
      options: z.object({
        effort: effortOption(),
        // open-models fork: --provider on patch
        provider: SKILL_PROVIDER_OPTION,
      ...
      async run({ format, options }) {
        try {
          ...
          exitCode = await runSkill(
            ...
            dependencies,
            // open-models fork: forward --provider
            { environment, provider: options.provider },
          );
        } catch (error) {
          exitCode = 2;
...
async function runSkill(
  ...
  // open-models fork: a registry provider reuses the fork's provider handling
  // in parseCodexOverrides (default model, full provider table). A
  // configuration-derived provider outside the registry is emitted verbatim
  // below; passing "openai" for it only suppresses the fork's
  // default-provider injection.
  const registryProvider =
    options.provider === "openai" ||
    options.provider === "amazon-bedrock" ||
    isExternalModelProvider(options.provider)
      ? (options.provider as ScanModelProviderName)
      : undefined;
  const overrides = parseCodexOverrides(
    codexOverrides,
    undefined,
    effort,
    registryProvider ?? (options.provider === undefined ? undefined : "openai"),
  );
  ...
  return dependencies.runCodex(
    [
      ...
      // open-models fork: registry providers carry the full provider table;
      // other configured providers use upstream's per-key emission.
      ...(isExternalModelProvider(skillProvider)
        ...
        : registryProvider === undefined && options.provider !== undefined
          ? [
              "--config",
              `model_provider=${JSON.stringify(options.provider)}`,
