...
| `sdk/typescript/package.json`                                                                               | `generate:pricing` script; `name` is `open-source-security`, `bin` is `open-security` (renamed launcher in `bin/`).                                                                                                                                                                                                                                                                             |
...
| `sdk/typescript/scripts/check-package.mjs`, `scripts/release-automation.mjs`                                | package-name checks accept `open-source-security`; the dist allowlist includes the fork modules `open-models` and `openrouter-pricing` (release automation accepts both names so its upstream tests keep passing).                                                                                                                                                                                                                                                    |
...
[`open-source-security`](https://www.npmjs.com/package/open-source-security)
(npm refused `open-security` as too similar to an existing package; the
installed command is still `open-security`). Releases are manual, from a
clean `main` checkout:
...
pnpm run check:package ./open-source-security-<version>.tgz
npm publish ./open-source-security-<version>.tgz  # as the jtippett npm account
```
